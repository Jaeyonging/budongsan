...
export async function GetHouseMonthData(code: string, date: string) {
    return axios.get(`http://openapi.molit.go.kr:8081/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcAptRent?LAWD_CD=${code}&DEAL_YMD=${date}&serviceKey=${service_Key}`).then((response) => {
        console.log(response)
        return response.data.response.body.items.item
    }).catch((err) => {
        console.log(err)
